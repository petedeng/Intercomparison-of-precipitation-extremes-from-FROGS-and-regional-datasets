...
def plot_spatial_patterns(spatial_patterns, datasets_names, boundary_kwargs, cluster_kwargs, txt_kwargs, reg_data_type='default'):
...
#    lat_map_ticks = boundary_kwargs.get('lat_map_ticks', default_deg)
    ...
    if (lon_reg_max_n - lon_reg_min_n) >= 120:
        lon_map_ticks = np.arange(lon_reg_min_n,lon_reg_max_n+0.1, 40)
    else:
        lon_map_ticks = np.arange(lon_reg_min_n,lon_reg_max_n+0.1, 30)
    ##
...
##    lat_reg_max = custom_round(lat_reg_max)
    ...
    for i, ax in enumerate(axs.axs.flat):
        ...
        if i > (num_reanal+num_sat+num_insitu) and i <= (num_reanal+num_sat+num_insitu+num_reg):
            print(i, datasets_names[i])
        ...
        if i >0 and i <= (num_insitu):
            print(i, datasets_names[i])
            ax.annotate(datasets_names[i], xy=(0.5, 1.1), xycoords='axes fraction',
                    fontsize=12, ha='center', va='center',
                    bbox=dict(boxstyle="round,pad=0.3", edgecolor='purple', facecolor='white',linewidth=1.2))
        if i >(num_insitu) and i <=(num_sat+num_insitu):
            print(i, datasets_names[i])
            ax.annotate(datasets_names[i], xy=(0.5, 1.1), xycoords='axes fraction',
                    fontsize=12, ha='center', va='center',
                    bbox=dict(boxstyle="round,pad=0.3", edgecolor='orange', facecolor='white',linewidth=1.2))

        if i >(num_sat+num_insitu) and i <=(num_reanal+num_sat+num_insitu):
            print(i, datasets_names[i])
            ax.annotate(datasets_names[i], xy=(0.5, 1.1), xycoords='axes fraction',
                    fontsize=12, ha='center', va='center',
    ...
    plt.figtext(0.5, 0.995, f"Map plots of {pr_indx_n} over {region_n} in the updated SREX regions in the sixth IPCC assessment report (AR6)", ha='center', wrap=True, fontsize=26)
    if reg_data_type=='default':
        txt = (f"Fig. 2. Map plots of {pr_indx_n} {pr_indx_n_unit} for the {base_p_n} climatological period over {region_n} ({ar6_reg_name}), which are based on the datasets from "
    ...
    # Your existing code with manual text wrapping
    max_line_length = 120  # Adjust the maximum line length as needed
    ...
    plt.figtext(0.01, 0.08, wrapped_txt, ha='left', va='top', fontsize=20) #
    
###    fig.text(-0.05, -0.1, wrapped_txt, ha='left', va='top', fontsize=13) #
###    
###    plt.figtext(0.01, 0.08, txt, wrap=True, ha='left', va='top', fontsize=20) #


def plot_spatial_patterns_diff(spatial_patterns_diff, datasets_names, boundary_kwargs, cluster_kwargs, txt_kwargs, reg_data_type='default'):
    default_value = 1  # Define a default number for cluster_kwargs
    default_deg = 1.  # Define a default number for cluster_kwargs
    default_title = 'Default Title'  # Define a default title for txt_kwargs
...
#    end_value = v_limit
    ...
    # Adjust start and end values to be multiples of interval_size_spatialp
    #start_value = math.floor(raw_start_value / interval_size_spatialp) * interval_size_spatialp
    end_value = math.ceil(v_limit / interval_size_spatialp) * interval_size_spatialp

    # Calculate number of colors
    num_colors = int((end_value) / interval_size_spatialp) *2
    print(num_colors)
    ...
    levels_pr = np.linspace(end_value*(-1), end_value, num_colors + 1)
...
#    lat_reg_max = custom_round(lat_reg_max)
    ...
    plt.figtext(0.5, 0.995, f"Map plots of difference for {pr_indx_n} between each dataset and the multi-data mean over {region_n}", ha='center', wrap=True, fontsize=26)
...
#    ax1.tick_params(bottom=False)  # remove the ticks
    ...
    for i, d in enumerate(data_clim_arr1):
        ...
        # Add text label near each marker
        #label_offset = 1.  # Adjust this value as needed
        ax1.text(x_pos + 0.1, d, f'{i+1}', fontsize=9, verticalalignment='center')  # Adjust the position and font size as needed
    ...
    for i, d in enumerate(data_clim_arr2):
        ...
        ax1.text(x_pos + 0.1, d, f'{i+1+num_insitu}', fontsize=9, verticalalignment='center')  # Adjust the position and font size as needed
    ...
    for i, d in enumerate(data_clim_arr4):
        ...
        ax1.text(x_pos + 0.1, d, f'{i+1+num_insitu+num_sat}', fontsize=9, verticalalignment='center') 
    ...
    for i, d in enumerate(data_clim_arr5):
        ...
        ax1.text(x_pos + 0.1, d, f'{i+1+num_insitu+num_sat+num_reanal}', fontsize=9, verticalalignment='center') 
...
#    ax2.tick_params(bottom=False)  # remove the ticks
    ...
    for i, d in enumerate(data_clim_arr1):
        ...
        ax2.text(x_pos + 0.1, d, f'{i+1}', fontsize=9, verticalalignment='center')
...
#    plt.autoscale()
    ...
    # Use textwrap to automatically wrap the text
    ##wrapped_txt = textwrap.fill(txt, width=80)

    # Your existing code with manual text wrapping
    max_line_length = 160  # Adjust the maximum line length as needed
    lines = txt.splitlines()
    wrapped_lines = []
...
###    txt1 = fig.text(-0.05, -0.1, txt, wrap=True, ha='left', va='top', fontsize=13) #
###    txt1._get_wrap_line_width = lambda : 600.  #  wrap to 600 screen pixels
